...
static Eigen::MatrixXd BinaryCrossEntropySingleError(Eigen::MatrixXd predicted,
                                                     Eigen::MatrixXd expected) {
  assert(predicted.size() == 1 && expected.size() == 1
         && "Binary cross entropy requires one value for the result and "
            "prediction");
...
static Eigen::MatrixXd BinaryCrossEntropyError(Eigen::MatrixXd predicted,
                                               Eigen::MatrixXd expected) {
  Eigen::MatrixXd res(1, 1);
...
class Network {
 public:
  Network(size_t input_size, size_t hidden_size, size_t output_size,
          bool bias = true, bool randomize = true,
  ...
  const Eigen::MatrixXd &Backpropagate(const Eigen::MatrixXd &targets,
